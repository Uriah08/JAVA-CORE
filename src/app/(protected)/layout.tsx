...
const ProtectedLayout = ({ children }: Props) => {
  ...
  const [active, setActive] = React.useState('/create-job');

  const [open, setOpen] = React.useState(true);

  return (
    <div className='h-full w-screen flex bg-[#eee8e8]'>
      <div className={`h-screen fixed p-7 bg-main flex flex-col justify-between min-w-[269px] ${open ? '' : '-left-[269px]'}`}>
        ...
        <div className='flex flex-col w-full'>
          ...
          <div className='flex flex-col w-full mt-2 gap-1'>
            {sidebar.map((item) => (
              ...
              {item.title === 'Profile' && (<h1 className='text-[#FFADAD] font-medium text-sm mt-2'>Other</h1>)}
              <Link onClick={() => setActive(item.link)} href={item.link} key={item.link} className={`flex gap-2 pl-8 relative py-3 rounded-lg duration-200 transition-all ${active === item.link ? 'bg-white' : 'hover:bg-white hover:bg-opacity-20'}`}>
              <item.icon size={20} className={`${active === item.link ? 'text-main' : 'text-white'}`}/>
              <h1 className={`font-medium text-sm ${active === item.link ? 'text-main' : 'text-white'}`}>{item.title}</h1>
              {active === item.link && (<div className='absolute bg-[#FFADAD] h-full -right-[28px] top-0 z-99 w-3 rounded-s-lg'/>)}
            </Link>
            </React.Fragment>
            ))}
          </div>
      ...
      <div className='flex flex-col gap-3 w-full'>
        ...
        <div className='bg-white p-3 rounded-lg'>
          <div className='flex gap-3 items-center'>
            <CircleHelp size={30} className='text-white bg-main rounded-sm p-1'/>
            <h1 className='font-medium'>Need Help?</h1>
          </div>
          <Image src={'/contact-logo.png'} width={200} height={200} alt='help' className='object-center size-36 object-contain -mt-5 ml-3'/>
          <button className='bg-main w-full text-white rounded-lg py-1 -mt-10'>Contact Dev</button>
        </div>
      </div>
      </div>
